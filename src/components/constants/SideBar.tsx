...
const SideBar = () => {
  ...
  return (
    <div className={`${theme.t1} ${theme.bg} durat00 tab:w-[220px] tab:max-w-[220px] ${show && 'w-[170px] h-screen tab:h-full absolute tab:static py-3 tab:py-0 top-0 left-0'} text-sm justify-between flex flex-col`} >
        <div className="flex w-full flex-col h-full">
            <div className={`flex px-3 items-center`}>
              <BiMenu onClick={()=> setShow(!show)} className='mx-2 ml-0 text-2xl cursor-pointer tab:hiden' />
              <img className={`w-[100px] tab:flex hidden ${show && 'flex'}`} src="src/assets/logoq.png" alt="" />
            </div>
            <div className={`flex tab:mt-5 mt-7 ${show && 'mt-5'} w-full flex-col h-full`}>
              ...
              <div className="flex mt-5 w-full flex-col">
                <div className={`tab:flex items-center justify-between ${theme.text} hidden ${show && 'flex'} px-3 w-full font-bold`}>
                  <p>Pages</p>
                  <BiChevronDown />
                </div>
